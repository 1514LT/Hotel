...
int main(int argc, char const *argv[])
{
    ...
    if (strcmp(type, "name") == 0)
        ...
        sprintf(sql, "SELECT * FROM user WHERE card=\'%s\';", user);
    ...
    if (row >= 0)
    {
        ...
        {
            if (strcmp(type, "name") == 0)
                sprintf(sql, "select user,user.card,time from user,data where user.card=data.card and user.user=\'%s\';", user);
            else if (strcmp(type, "id") == 0)
                sprintf(sql, "select user,user.card,time from user,data where user.card=\'%s\' and data.card=\'%s\';", user, user);
            char **result = NULL;
            row = 0, col = 0;
