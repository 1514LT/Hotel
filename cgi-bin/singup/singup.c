...
int main(int argc, char const *argv[])
{
    ...
    char key[32] = "";
    //char card[125] = "";
    sscanf(data, "%[^:]:%s", usr,pwd_key);
    sscanf(pwd_key, "%[^:]:%s",pwd,key);
    //获取数据库中的数据
    //创建TCP套接字
    ...
    sqlite3_get_table(db,sql,&key_result,&row,&col,NULL);
    if(row<1)
    {
        //key验证不合格
        printf("key_erro");
    ...
    {
        memset(sql, 0, 128);
        sprintf(sql, "insert into user values(\'%s\',\'%s\',\'%s\',\'user\');",usr,pwd,card);
        //printf("%s\n", sql);
        sqlite3_exec(db,sql,NULL,NULL,NULL);
        memset(sql, 0, sizeof(sql));
        sprintf(sql, "INSERT INTO account VALUES (\'%s\',\'%s\',0,\'%s\');", usr, pwd, card);
        sqlite3_exec(db, sql, NULL, NULL, NULL);
        printf("1");
    }
